...
def load_data(file_path:str)->pd.DataFrame:
    ...
    try:
        ...
        logger.error('Failed to parse the CSVfile: %s',e)
        raise
    except Exception as e:
...
def get_root_directory()->str:
    """Get the root directory (two levels up from this scripts loaction)."""
...
def apply_tfidf(train_data:pd.DataFrame,max_features:int,ngram_range:tuple)->tuple:
    ...
    try:
        ...
        logger.debug(f"TF-IDF transformation complete.Treain shape: {X_train_tfidf.shape}")
...
def train_lgbm(X_train:np.ndarray,y_train:np.ndarray,learning_rate:float,max_depth:int,n_estimators:int)->None:
    ...
    try:
        ...
        logger.debug('LoghtGBM model training completed')
        return best_model
...
def main():
    try:
        ...
        #Train the LightGBM using hyperparameters from params.yaml
